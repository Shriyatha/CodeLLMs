...
def get_execution_service():
    try:
        ...
        logger.error(f"Failed to initialize CodeExecutionService: {e!s}")
        raise HTTPException(
...
def get_course_loader():
    try:
        ...
        logger.error(f"Failed to initialize CourseLoader: {e!s}")
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
...
def get_llm_service():
    try:
        ...
        logger.error(f"Failed to initialize LLMService: {e!s}")
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
...
async def validate_problem_exists(course_id: str, topic_id: str, problem_id: str, loader: CourseLoader):
    ...
    try:
        ...
        logger.error(f"Error loading problem: {e!s}")
        raise HTTPException(
...
async def get_topic_details(
    ...
    try:
        ...
        logger.error(f"Error getting topic details: {e!s}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_problem_details(
    ...
    try:
        ...
        logger.error(f"Error getting problem details: {e!s}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Internal server error",
        )
...
async def execute_problem(
    ...
    try:
        ...
        logger.error(f"Execution failed: {e!s}", exc_info=True)
        raise HTTPException(
            status_code=500,
...
async def get_progressive_hints(
    ...
    try:
        ...
        logger.error(f"Hint generation failed: {e!s}", exc_info=True)
        raise HTTPException(
            status_code=500,
...
async def explain_errors(
    ...
    try:
        ...
        logger.error(f"Error explanation failed: {e!s}", exc_info=True)
        raise HTTPException(
            status_code=500,
...
async def suggest_optimizations(
    ...
    try:
        ...
        logger.error(f"Optimization suggestion failed: {e!s}", exc_info=True)
        raise HTTPException(
            status_code=500,
...
async def get_conceptual_steps(
    ...
    try:
        ...
        logger.error(f"Failed to generate steps: {e!s}", exc_info=True)
        raise HTTPException(
            status_code=500,
...
async def get_debugging_tips(
    ...
    try:
        ...
        logger.error(f"Debugging failed: {e!s}", exc_info=True)
        raise HTTPException(
            status_code=500,
...
async def generate_pseudocode(
    ...
    try:
        ...
        logger.error(f"Pseudocode failed: {e!s}", exc_info=True)
        raise HTTPException(
            status_code=500,
...
async def run_flow_in_background(flow_func, *args, **kwargs):
    ...
    try:
        ...
        logger.error(f"Flow execution failed: {e!s}", exc_info=True)
...
async def submit_problem(
    ...
    try:
        ...
        logger.error(f"Grading failed: {e!s}")
        await SubmissionStorage.update(submission_id, {
            "status": "failed",
            "result": {"error": str(e)},
            "updated_at": datetime.now(UTC).isoformat(),
